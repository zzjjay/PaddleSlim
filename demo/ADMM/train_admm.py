...
add_arg('l2_decay',         float,  5e-4,               "The l2_decay parameter. Default: 3e-5")
...
add_arg('model_path',       str, "./admm_models_mbv1",         "The path to save model. Default: ./models")
...
add_arg('rho', type=float, default=1e-2,   help='cardinality weight (default: 1e-3)')
# yapf: enable
...
def initialize_Z_U(program, args):
    Z = []
    U = []
    idx = 0
    percent = 100*args.ratio
...
def print_convergence(program, Z):
    ...
    for param in program.list_vars():
            ...
            norm_value = norm(w - Z[idx])
            norm_value2 = norm(w - Z[idx]) / norm(w)
            print("{}, {}: {:.4f} {}".format(idx, param.name, norm_value, norm_value2))
            idx += 1
...
def compress(args):
    ...
    if args.last_epoch > -1:
        ...
        print("prtrained model0")
        def if_exist(var):
            return os.path.exists(os.path.join(args.pretrained_model, var.name))
        print("prtrained model1")
        _logger.info("Load pretrained model from {}".format(
            args.pretrained_model))
        print("pretrained model2")
    ...
    def ADMM_train(args, program):
        ...
        Z_value, U_value = initialize_Z_U(train_program, args)

        for i in range(args.last_epoch + 1, args.num_epochs):
            # for i in range(4):
            #     print("Z_value:{} {} {}".format(np.count_nonzero(Z_value[i])/np.product(Z_value[i].shape), np.sum(Z_value[i]), np.mean(Z_value[i])) )
            #     print("U_value:{} {} {}".format(np.count_nonzero(U_value[i])/np.product(U_value[i].shape), np.sum(U_value[i]), np.mean(U_value[i])) )
            ...
            for batch_id, data in enumerate(train_loader):
                ...
                loss_n, acc_top1_n, acc_top5_n = exe.run(
                    program,
                    feed=data,
                    fetch_list=[admm_loss.name, acc_top1.name, acc_top5.name])
