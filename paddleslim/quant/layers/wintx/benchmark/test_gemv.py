...
@triton.testing.perf_report(
    triton.testing.Benchmark(
        ...
        x_vals=[i for i in range(1,16,2)],  # different possible values for `x_name`
...
def benchmark(provider, M, N, K, dtype,asymm, warmup=16):
    ...
    inp_paddle = paddle.randn((M,K)).astype(dtype)
    w_paddle = paddle.randn((K,N)).astype(dtype)
    ...
    if provider == 'paddle_triton':
        ...
        for _ in range(warmup):
            gemv_int2_paddle(inp_paddle, pack_w, scales, pack_zp, group_size=32)
        # ms = triton.testing.do_bench(lambda: matmul_dequantize_int2_v2(inp, pack_w, scale, pack_zp))
        ms = triton.testing.do_bench(lambda: gemv_int2_paddle(inp_paddle, pack_w, scales, pack_zp, group_size=128),warmup=1000, rep=1000,)
