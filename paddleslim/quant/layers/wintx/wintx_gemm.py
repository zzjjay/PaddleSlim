...
__all__ = ['gemm_int2_paddle','gemm_int4_paddle',
            'gemm_unpack_int2_paddle','gemm_unpack_int4_paddle',
]
...
@custom_autotune.autotune(
        configs=get_wintx_kernel_config(),
        key=["M", "N", "K", "n_bit"],
...
if __name__ == '__main__':
    ...
    inp,w = paddle.randn([64,7168]).cast('float16'),paddle.randn([7168,4096]).cast('float16')
    ...
    latency,_,_ = eval_time_paddle(lambda:gemm_int2_paddle(inp, pack_w, scale))
    ...
    pack_w = pack_w.T
    pack_w = pack_w.contiguous()
    latency,_,_ = eval_time_paddle(lambda:gemm_int4_paddle(inp, pack_w, scale))
