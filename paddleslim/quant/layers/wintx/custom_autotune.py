...
class CustomizedTritonAutoTuner(triton.KernelInterface):
    ...
    def _bench(self, *args, config, **meta):
        ...
        try:
            ...
            minv,meanv,maxv = eval_time_paddle(kernel_call, rep=40)
            return minv,meanv,maxv
        except triton.OutOfResources:
            print("[Triton Autotune]Out Of Resources")
    ...
    def run(self, *args, **kwargs):
        ...
        if len(self.configs) > 1:
            ...
            if self.nearest_power_of_two:
                key = tuple([2 ** int(math.log2(x) + 0.5) if isinstance(x, int) else x for x in key])
                # import pdb;pdb.set_trace()
...
def kernel_config_pruner(configs, nargs):
    """
