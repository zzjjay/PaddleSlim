...
def random_hadamard_matrix(size, device=None, ffn2=False):
    """
    生成一个随机的Hadamard矩阵，该矩阵是一个对角线矩阵，其中每个元素为-1或1。
    如果ffn2参数为True，则返回一个大小为size的矩阵，其中每个块都是一个大小为size/2的Hadamard矩阵。
    
    Args:
        size (int): 矩阵的大小。
        device (str, optional): 可选的设备。
        ffn2 (bool, optional): 布尔值，指示是否返回大小为size的矩阵，默认为False。
